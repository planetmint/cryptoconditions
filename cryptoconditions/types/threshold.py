...
class ThresholdSha256(BaseSha256):
    ...
    def __init__(self, threshold=None):
        ...
        if threshold is not None and (not isinstance(threshold, int) or threshold < 1):
            raise ValueError(
                "Threshold must be a integer greater than zero, was: {}".format(
                    threshold
                )
            )
    ...
    def asn1_dict_payload(self):
        ...
        SF = [
            {mf["body"].TYPE_ASN1: mf["body"].asn1_dict_payload}
    ...
    def get_subcondition_cost(subcondition):
        return (
            subcondition["body"].condition.cost
    ...
    def calculate_smallest_valid_fulfillment_set(threshold, fulfillments, state=None):
        ...
        if threshold <= 0:
            ...
            return (
                with_next if with_next["size"] < without_next["size"] else without_next
            )
        else:
    ...
    def calculate_cost(self):
        ...
        subcondition_costs = [
            ThresholdSha256.get_subcondition_cost(s) for s in self.subconditions
        ]

        worst_case_fulfillments_cost = ThresholdSha256.calculate_worst_case_length(
            self.threshold, subcondition_costs
        )
