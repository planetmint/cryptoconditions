...
class TestBigchainCryptoED25519(object):
    ...
    def test_valid_condition_valid_signature_ilp(self, vk_ilp, signature):
        ...
        assert (
            vk.verify(
                msg,
                binascii.hexlify(base64.b64decode(signature["sig"])),
                encoding="hex",
            )
            is True
        )
        assert (
            vk.verify(msg, base64.b64decode(signature["sig"]), encoding="bytes") is True
        )
    ...
    def test_valid_condition_invalid_signature_ilp(self, vk_ilp, signature):
        ...
        assert (
            vk.verify(
                msg,
                binascii.hexlify(base64.b64decode(signature["msg"])),
                encoding="hex",
            )
            is False
        )
        assert (
            vk.verify(msg, base64.b64decode(signature["msg"]), encoding="bytes")
            is False
        )
