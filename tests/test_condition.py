...
def test_from_uri_prefix_error(uri):
    ...
    assert exc_info.value.args == (
        'Serialized condition must start with "{}:"'.format(CONDITION_URI_SCHEME),
    )
...
@given(
    from_regex(r"^ni:\/\/\/sha-256;([a-zA-Z0-9_-]{0,86})\?fpt=preimage-sha-256&(.+)$")
)
...
def test_condition_comparison(minimal_ed25519, minimal_prefix):
    ...
    assert Condition.from_uri(minimal_ed25519.condition_uri) == Condition.from_uri(
        minimal_ed25519.condition_uri
    )

    assert not Condition.from_uri(minimal_ed25519.condition_uri) == Condition.from_uri(
        minimal_prefix.condition_uri
    )
